<?php // $Id: importclasses.php,v 1.3 2009/11/12 10:48:45 Shtifanov Exp $
			  ...
			  foreach ($symbols as $sym)	{
				...
				if(!$class = get_record('monit_school_class', 'schoolid', $sid, 'yearid', $yid, 'name', $pupil->name)) 	 {
						...
						if ($idnew = insert_record('monit_school_class', $rec))	{
						    ...
						    print_r($rec); echo '<hr>';
							error(get_string('errorinaddingclass','block_mou_ege'), "importclasses.php?sid=$sid&amp;rid=$rid&amp;yid=$yid");
						}
                }
...
//   }
	...
	if ($rid!=0 && $sid!=0) {
		...
		print_tabs_years_link("importclasses.php?", $rid, $sid, $yid);
