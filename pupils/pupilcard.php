<?php // $Id: pupilcard.php,v 1.3 2009/12/08 08:22:17 Shtifanov Exp $
        ...
        if (find_regform_errors($user, $usernew, $err, $um, $profile)) {
			...
			if ($disciplines)	{
				...
				foreach ($disciplines as $discipline) 	{
					$pf = 'disc_mi_'.$discipline->id;
					if (isset($usernew->{$pf}) && $usernew->{$pf} == 1)  {
						$listmiids .= $discipline->id . ',';
						$pfdate = 'date_mi_'.$discipline->id;
						$listmidatesids .= $usernew->{$pfdate} . ',';
					}
				}
				$listmiids .= '0';
				$pupil->listmiids = $listmiids;
				$listmidatesids .= '0';
				$pupil->listmidatesids = $listmidatesids;
			}
            ...
            if (update_record("user", $usernew)) {
                ...
                if ($user->id == $USER->id) {
                    ...
                    if (!empty($SESSION->wantsurl)) {  // User may have been forced to edit account, so let's
                        ...
                        redirect("$CFG->wwwroot/blocks/mou_mi/class/classpupils.php?rid=$rid&amp;sid=$sid&amp;yid=$yid&amp;gid=$gid", get_string("changessaved"), 0);
                    }
                } else {
                    redirect("$CFG->wwwroot/blocks/mou_mi/class/classpupils.php?rid=$rid&amp;sid=$sid&amp;yid=$yid&amp;gid=$gid", get_string("changessaved"), 0);
                }
